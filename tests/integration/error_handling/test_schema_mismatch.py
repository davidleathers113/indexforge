...
def test_field_type_mismatch():
    ...
    doc = create_valid_document()
    type_mismatches = [('content_body', 123), ('schema_version', '1'), ('chunk_ids', 'not_a_list'), ('embedding', 'not_a_vector')]
...
def test_metadata_schema_mismatch():
    ...
    doc = create_valid_document()
    invalid_metadata = [{'key': 'value'}, 123, 'string_metadata', {'nested': {'too': {'deep': {'for': 'schema'}}}}]
...
def test_relationship_schema_mismatch():
    ...
    doc = create_valid_document()
    invalid_relationships = [{'parent_id': [1, 2, 3]}, {'chunk_ids': 'chunk1'}, {'parent_id': {'id': 'parent1'}}, {'chunk_ids': [1, '2', 3.0]}]
...
def test_timestamp_format_mismatch():
    ...
    doc = create_valid_document()
    invalid_timestamps = ['2024-01-20', '12:00:00', '2024-01-20 12:00:00', '2024-13-45T25:61:61Z', datetime.now().timestamp()]
...
def test_content_format_mismatch():
    ...
    doc = create_valid_document()
    invalid_contents = [b'binary data', ['content', 'in', 'list'], {'content': 'in_dict'}, 123, True]
