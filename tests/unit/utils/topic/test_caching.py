...
def sample_documents():
    ...
    return [
        {
            "content": {"body": "test document 1"},
            "embeddings": {"body": [0.1, 0.2, 0.3]},
            "metadata": {},
        },
        {
            "content": {"body": "test document 2"},
            "embeddings": {"body": [0.2, 0.3, 0.4]},
            "metadata": {},
        },
        {
            "content": {"body": "test document 3"},
            "embeddings": {"body": [0.7, 0.8, 0.9]},
            "metadata": {},
        },
    ]
...
def test_different_configs(clusterer, sample_documents, mock_cache_manager):
    """Test caching with different configurations"""
    from src.utils.topic_clustering import ClusteringConfig

    # First clustering with default config
    clusterer.cluster_documents(sample_documents)

    # Second clustering with different config
    config = ClusteringConfig(n_clusters=3)
    clusterer.cluster_documents(sample_documents, config=config)

    # Verify cache was checked at least twice
