...
def test_custom_parameters(pipeline_with_mocks, tmp_path):
    ...
    export_dir = tmp_path / 'test_dir'
    export_dir.mkdir()
    pipeline = run_pipeline(export_dir=str(export_dir), index_url='http://custom:8080', log_dir='custom_logs', batch_size=50, cache_host='custom_host', cache_port=6380, cache_ttl=3600)
    assert pipeline._export_dir == export_dir
...
def test_parameter_validation(pipeline_with_mocks, tmp_path):
    ...
    with pytest.raises(ValidationError, match='batch_size must be a positive integer'):
        run_pipeline(export_dir=str(tmp_path), index_url='http://localhost:8080', batch_size=0)
    with pytest.raises(ValidationError, match='cache_port must be between 0 and 65535'):
        run_pipeline(export_dir=str(tmp_path), index_url='http://localhost:8080', cache_port=70000)
    with pytest.raises(ValidationError, match='cache_ttl must be a positive integer'):
        run_pipeline(export_dir=str(tmp_path), index_url='http://localhost:8080', cache_ttl=-1)
...
def test_parameter_types(pipeline_with_mocks, tmp_path):
    ...
    with pytest.raises(ValidationError, match='batch_size must be int'):
        run_pipeline(export_dir=str(tmp_path), index_url='http://localhost:8080', batch_size='not a number')
    with pytest.raises(ValidationError, match='cache_port must be int'):
        run_pipeline(export_dir=str(tmp_path), index_url='http://localhost:8080', cache_port='not a number')
    with pytest.raises(ValidationError, match='cache_ttl must be int'):
        run_pipeline(export_dir=str(tmp_path), index_url='http://localhost:8080', cache_ttl='not a number')

def test_argument_parsing(mock_argparse_parse_args, mock_cli_state, pipeline_with_mocks, tmp_path):
    """Test CLI argument parsing."""
    export_dir = tmp_path / 'test_dir'
    export_dir.mkdir()
    mock_cli_state.set_args(export_dir=str(export_dir), index_url='http://custom:8080', log_dir='custom_logs', batch_size=50, cache_host='custom_host', cache_port=6380, cache_ttl=3600)
...
def test_argument_parsing_error_handling(mock_argparse_parse_args, mock_cli_state, pipeline_with_mocks):
    """Test handling of argument parsing errors."""
    mock_cli_state.error_mode = True
    with pytest.raises(ValueError, match='Argument parsing failed in error mode'):
