...
def test_error_propagation(tmp_path):
    ...
    with patch('src.main.Pipeline') as mock_pipeline:
        ...
        mock_instance.process_documents.side_effect = ValueError('Invalid value')
        with pytest.raises(ValueError, match='Invalid value'):
            pipeline = run_pipeline(export_dir=str(test_dir), index_url='http://localhost:8080')
            pipeline.process_documents()
        mock_instance.process_documents.side_effect = RuntimeError('Runtime error')
        with pytest.raises(RuntimeError, match='Runtime error'):
            pipeline = run_pipeline(export_dir=str(test_dir), index_url='http://localhost:8080')
            pipeline.process_documents()
        mock_instance.process_documents.side_effect = Exception('Generic error')
...
def test_validation_error_handling(tmp_path):
    ...
    with patch('src.main.Pipeline') as mock_pipeline:
        ...
        with pytest.raises(ValidationError, match='batch_size must be a positive integer'):
            run_pipeline(export_dir=str(test_dir), index_url='http://localhost:8080', batch_size=0)
...
def test_pipeline_error_handling(tmp_path):
    ...
    with patch('src.main.Pipeline') as mock_pipeline:
        ...
        with pytest.raises(RuntimeError, match='Pipeline initialization failed'):
            run_pipeline(export_dir=str(test_dir), index_url='http://localhost:8080')
...
def test_error_cleanup(tmp_path):
    ...
    with patch('src.main.Pipeline') as mock_pipeline:
        ...
        pipeline = run_pipeline(export_dir=str(test_dir), index_url='http://localhost:8080')
        with pytest.raises(ValueError), patch('src.pipeline.context.managed_pipeline') as mock_context:
            cm = MagicMock()
            cm.__enter__.return_value = pipeline
            cm.__exit__.side_effect = lambda exc_type, exc_val, exc_tb: (mock_instance.cleanup(), False)[-1]
