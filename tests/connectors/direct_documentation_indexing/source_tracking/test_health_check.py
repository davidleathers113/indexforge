...
def test_health_check_healthy(storage):
    ...
    add_processing_step(storage, doc_id=doc_id, step_name='process', status=ProcessingStatus.SUCCESS)
...
def test_health_check_with_thresholds(storage):
    ...
    custom_thresholds = {'memory_warning': 70.0, 'memory_critical': 90.0, 'cpu_warning': 75.0, 'cpu_critical': 95.0}
    with patch('psutil.Process') as mock_process:
