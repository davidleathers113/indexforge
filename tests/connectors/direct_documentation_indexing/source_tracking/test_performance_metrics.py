...
def test_real_time_status(storage):
    ...
    add_processing_step(
        storage, doc_id=doc_id, step_name="extract", status=ProcessingStatus.SUCCESS
    )
    add_processing_step(
        storage, doc_id=doc_id, step_name="transform", status=ProcessingStatus.IN_PROGRESS
    )
...
def test_aggregated_metrics(storage):
    ...
    add_processing_step(
        ...
        details={"metrics": {"processing_time": 100, "memory_usage": 500}},
    )
    add_processing_step(
        storage,
        doc_id=doc_id,
        step_name="process",
        status=ProcessingStatus.SUCCESS,
        details={"metrics": {"processing_time": 150, "memory_usage": 600}},
    )
...
def test_metrics_persistence(temp_lineage_dir):
    ...
    add_processing_step(
        ...
        step_name="process",
        status=ProcessingStatus.SUCCESS,
        details={"metrics": {"processing_time": 100}},
    )
...
def test_metrics_with_multiple_documents(storage):
    ...
    doc_ids = ["doc1", "doc2"]
    metrics_data = {
        "doc1": {"processing_time": 100, "memory_usage": 500},
        "doc2": {"processing_time": 200, "memory_usage": 700},
    }
    for doc_id in doc_ids:
        ...
        add_processing_step(
            ...
            step_name="process",
            status=ProcessingStatus.SUCCESS,
            details={"metrics": metrics_data[doc_id]},
        )
    metrics = get_aggregated_metrics(storage.get_all_lineage())
...
def test_metrics_with_time_range(storage):
    ...
    add_processing_step(
        ...
        step_name="process2",
        status=ProcessingStatus.SUCCESS,
        details={"metrics": {"processing_time": 200}},
...
def test_real_time_status_updates(storage):
    ...
    add_processing_step(
        storage, doc_id=doc_id, step_name="step2", status=ProcessingStatus.IN_PROGRESS
    )
    status = get_real_time_status(storage, doc_id)
